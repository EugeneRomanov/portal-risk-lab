      ...
      {view === 'dashboard' && (
        <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          <div className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex items-center gap-3"><p className="font-mono text-[11px] text-slate-500">08 СЕН 2026 · 21:47 ICT</p><Button onClick={clearPortals} variant="ghost" size="sm" className="text-slate-500 hover:bg-rose-300/8 hover:text-rose-200"><Trash2 />Очистить</Button></div>
          </div>
...
function Worklog({ onBack }: { onBack: () => void }) {
  const stages = [
    ['Разбор задания', 'Человек выбрал вариант и подтвердил рамки. AI сравнил задания, выделил критерии оценки и предложил минимальный законченный объём.', '«Проанализируй два задания и помоги выбрать одно для решения».'],
    ['Архитектура и риск-логика', 'Человек утвердил направление. AI отделил чистые функции расчёта и правил от интерфейса, подготовил тестируемую модель состояний.', '«Сделай прозрачную формулу риска, допустимые переходы и обработай обязательные edge cases».'],
    ['Интерфейс', 'AI собрал рабочую поверхность оператора: сводку, фильтры, карточку портала, предупреждения, журнал и адаптивную компоновку.', '«Интерфейс должен быть понятен за минуту и ощущаться законченным инструментом».'],
    ['Проверка и завершение', 'AI добавил автоматические тесты, ручной чеклист, локальное сохранение и документацию запуска. Человек проверяет финальную ссылку перед сдачей.', '«Проверь критический риск, запрещённые действия, изменение риска, журнал и пустой список».'],
  ];
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6"><p className="eyebrow"><Sparkles /> Прозрачность AI-процесса</p><div className="mb-7 flex items-end justify-between gap-4"><div><h1 className="page-title">AI Worklog</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Честный журнал того, как AI использовался для проектирования, реализации и проверки продукта.</p></div><Button onClick={onBack} variant="outline" className="border-white/10 bg-white/[0.025]">К обзору<ArrowRight /></Button></div>
    <section className="mb-5 grid gap-3 sm:grid-cols-3"><InfoCard label="AI-инструменты" value="Codex · GPT-5" note="Код, UI, тесты, документация" /><InfoCard label="Время разработки" value="≈ 2 часа" note="От анализа ТЗ до публикации" /><InfoCard label="Токены" value="Не считались" note="Интерфейс не показывает точную статистику" /></section>
    <section className="surface-card mb-5 rounded-2xl p-5 sm:p-6"><div className="mb-5 flex items-center gap-3"><BookOpenCheck className="size-5 text-cyan-300" /><div><h2 className="text-sm font-semibold">Этапы и ключевые промпты</h2><p className="mt-1 text-xs text-slate-500">Что делал человек, что делал AI</p></div></div><div className="grid gap-3 md:grid-cols-2">{stages.map(([title, body, prompt], index) => <article key={title} className="rounded-xl border border-white/7 bg-black/15 p-4"><p className="mb-3 font-mono text-[10px] text-cyan-300">0{index + 1}</p><h3 className="text-sm font-medium">{title}</h3><p className="mt-2 text-xs leading-5 text-slate-400">{body}</p><div className="mt-3 rounded-lg border border-white/6 bg-white/[0.025] p-3"><p className="mb-1 text-[9px] uppercase tracking-[.13em] text-slate-600">Суть промпта</p><p className="text-[11px] leading-5 text-slate-400">{prompt}</p></div></article>)}</div></section>
    <div className="grid gap-5 lg:grid-cols-2"><WorklogList icon={CheckCircle2} title="Решения, принятые человеком" items={['Выбрать «Лабораторию порталов» вместо более сложного глобального мэтчинга привидений.', 'Ограничить продукт одной рабочей поверхностью без сервера и авторизации.', 'Сделать формулу риска видимой, а не скрытой «магией».', 'Сохранять отклонённые попытки в журнале для полноценного аудита.', 'Использовать локальное хранение как честную границу демонстрационной версии.']} /><WorklogList icon={AlertTriangle} title="Ошибки AI и ручные корректировки" items={['Первый вариант был слишком похож на обычный dashboard — визуальную иерархию пришлось усилить.', 'Изначально запрещённые кнопки предлагалось отключать; исправлено, чтобы пользователь получал объяснение причины.', 'Закрытие портала с существами сначала блокировалось полностью; заменено на осознанное подтверждение.', 'Формула риска упрощена до четырёх объяснимых факторов вместо непрозрачной эвристики.']} /><WorklogList icon={FlaskConical} title="Как проверялось" items={['Автотест критического риска.', 'Автотест снижения риска после стабилизации.', 'Автотест запрета наблюдателя при критическом риске.', 'Автотест подтверждения закрытия с существами.', 'Автотест запрета действий с закрытым порталом.', 'Ручная проверка пустого списка, фильтров, сохранения и журнала.']} /><WorklogList icon={Sparkles} title="Что улучшить в реальном продукте" items={['Серверная база данных и роли операторов.', 'События в реальном времени и синхронизация смен.', 'Настраиваемые пороги риска с версионированием формулы.', 'Отмена операций и защищённый аудит изменений.', 'Доступность по WCAG и e2e-тесты на основных устройствах.']} /></div>
  </div>;
}
