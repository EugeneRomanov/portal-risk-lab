...
function Worklog({ onBack }: { onBack: () => void }) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6"><p className="eyebrow"><Sparkles /> Прозрачность AI-процесса</p><div className="mb-7 flex items-end justify-between gap-4"><div><h1 className="page-title">AI Worklog</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Честный журнал того, как AI использовался для проектирования, реализации и проверки продукта.</p></div><Button onClick={onBack} variant="outline" className="border-white/10 bg-white/[0.025]">К обзору<ArrowRight /></Button></div>
    <section className="mb-5 grid gap-3 sm:grid-cols-3"><InfoCard label="AI-инструменты" value="AI-ассистент OpenAI · GPT-5" note="Анализ, код, UI, тесты и документация" /><InfoCard label="Время разработки" value="≈ 3 часа" note="Активная работа от анализа до публикации" /><InfoCard label="Токены" value="Не считались" note="Инструмент не показывает точную статистику" /></section>
    <section className="surface-card mb-5 rounded-2xl p-5 sm:p-6"><div className="mb-5 flex items-center gap-3"><BookOpenCheck className="size-5 text-cyan-300" /><div><h2 className="text-sm font-semibold">Этапы и ключевые промпты</h2><p className="mt-1 text-xs text-slate-500">Ответственность человека и AI показана отдельно</p></div></div><div className="grid gap-3 md:grid-cols-2">{stages.map((stage, index) => <article key={stage.title} className="rounded-xl border border-white/7 bg-black/15 p-4"><p className="mb-3 font-mono text-[10px] text-cyan-300">0{index + 1}</p><h3 className="text-sm font-medium">{stage.title}</h3><div className="mt-3 space-y-2 text-xs leading-5 text-slate-400"><p><strong className="font-medium text-slate-200">Моя роль:</strong> {stage.human}</p><p><strong className="font-medium text-slate-200">Роль AI:</strong> {stage.ai}</p></div><div className="mt-3 rounded-lg border border-white/6 bg-white/[0.025] p-3"><p className="mb-1 text-[9px] uppercase tracking-[.13em] text-slate-600">Суть промпта</p><p className="text-[11px] leading-5 text-slate-400">{stage.prompt}</p></div></article>)}</div></section>
    <div className="grid gap-5 lg:grid-cols-2"><WorklogList icon={CheckCircle2} title="Решения, принятые человеком" items={['Выбрать «Лабораторию порталов» вместо более сложного глобального мэтчинга привидений.', 'Ограничить продукт одной рабочей поверхностью без сервера и авторизации.', 'Сделать формулу риска видимой, а не скрытой «магией».', 'Сохранять отклонённые попытки в журнале для полноценного аудита.', 'Использовать локальное хранение как честную границу демонстрационной версии.']} /><WorklogList icon={AlertTriangle} title="Ошибки AI и корректировки" items={['Первый вариант был слишком похож на обычный dashboard — по результатам ревью усилена визуальная иерархия.', 'Изначально AI предлагал отключать запрещённые кнопки; решение изменено, чтобы пользователь видел причину отказа.', 'Закрытие портала с существами сначала блокировалось полностью; правило заменено на осознанное подтверждение.', 'Формула риска упрощена до четырёх объяснимых факторов вместо непрозрачной эвристики.', 'Ручного редактирования кода не было: человек управлял требованиями, решениями и проверкой, изменения вносил AI-ассистент.']} /><WorklogList icon={FlaskConical} title="Как проверялось" items={['Пять автоматических тестов доменной логики.', 'Production-сборка без ошибок.', 'Ручная проверка критического риска и рекомендации.', 'Ручная проверка отказа с объяснением и записью в журнале.', 'Ручная проверка снижения риска после стабилизации.', 'Ручная проверка предупреждения при закрытии с существами.', 'Проверка пустого состояния, восстановления данных и публичной ссылки.']} /><WorklogList icon={Sparkles} title="Что улучшить в реальном продукте" items={['Серверная база данных и роли операторов.', 'События в реальном времени и синхронизация смен.', 'Настраиваемые пороги риска с версионированием формулы.', 'Отмена операций и защищённый аудит изменений.', 'Доступность по WCAG и e2e-тесты на основных устройствах.']} /></div>
  </div>;
}
